...
cross.val.df <- na.omit(combo)

all.valid.date.times <- c(cross.val.df$Date[1])

0.05*difftime(min(cross.val.df$Date), max(cross.val.df$Date), units = "days")
...
colnames(my.cross.val.values) <- c("Date", "n.days", "perc.data", "RMSE_model_hyper", "R2_model_hyper", "p_model_hyper", "med_perc_error_hyper", "RMSE_model_hyper_full", "R2_model_hyper_full", "p_model_hyper_full", "med_perc_error_hyper_full", "mean.O2bio", "mean.O2biopred", "med.O2bio", "med.O2biopred", "mean.date", "mean.O2bio.full", "mean.O2biopred.full")
...
for(d in 1:length(all.valid.days)){
  ...
  range(cross.val.train$O2bio.estimated)
  range(cross.val.test$O2bio.estimated)
  ...
  summary(cross.val.test$O2bio.estimated)
  ## something is wrong here - vastly underestimating magnitudes
  ...
  my.cross.val.values$mean.O2bio.full[d] <- mean(cross.val.df$O2bio.estimated)
...
ggplot() +
  geom_line(aes(x = cross.val.df$Date[index], y = cross.val.test$O2bio.estimated), color = "blue") +
  geom_line(aes(x = cross.val.df$Date[index], y = temp.prediction), color = "red") +
  theme_bw()

ggplot() +
  geom_point(aes(x = cross.val.test$O2bio.estimated, y = temp.prediction), color = "red") +
  theme_bw()

saveRDS(combo, "2026-05-06_combo.rds")
